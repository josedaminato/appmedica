...
export function DashboardPage() {
  ...
  return (
    <div>
      ...
      {/* COBRANZAS — cuánto te deben */}
      <SectionTitle>Cobranzas</SectionTitle>
      <div className="grid gap-4 sm:grid-cols-3 mb-4">
        <MetricCard
      ...
      {alerts && alerts.top_debt_patients.items.length > 0 && (
        <Card className="mb-8">
          <CardHeader className="flex flex-row items-center justify-between">
      ...
      {/* MÁS DETALLES — info avanzada, oculta por defecto */}
      <Button
        variant="ghost"
        size="sm"
      ...
      {showDetails && (
        <div className="mt-4 space-y-6">
          ...
          {alerts && alerts.old_insurance_claims.items.length > 0 && (
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
              ...
              <CardContent>
                <ul className="space-y-2">
                  {alerts.old_insurance_claims.items.slice(0, 4).map((it) => (
                    <li
                      key={it.health_insurance_id}
                      className="flex items-center justify-between text-sm border-b last:border-0 pb-2 last:pb-0"
                    >
                      <div className="min-w-0">
                        <p className="font-medium truncate">{it.name}</p>
                        <p className="text-xs text-muted-foreground">
                          {it.claims_count} reclamos · {formatMoney(it.debt_total)} · prom{" "}
                          {it.avg_days_pending} d
                        </p>
                      </div>
...
function MetricCard({
  ...
  const content = (
    <Card className={cn("h-full", highlight && "border-amber-500/50 bg-amber-500/5")}>
